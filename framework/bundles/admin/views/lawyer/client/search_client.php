        ...
        <?php }?>
         <div class="span12">
            ...
            <a href="<?php echo URL::to_route('ViewClient'); ?>">
                    <button title="View all clients" class="btn btn-info" style="float: right"></i>View All</button>
            </a>
            ...
            <?php $client_name = libclient::getclientByLawyerID(Auth::user()->id); ?>
                <select id="client_id" class="span3" name="client_id" onchange='this.form.submit()'>
                  <?php if(isset($_GET['client_id'])){ $client_id=$_GET['client_id'];if($client_id){ ?>
                    <option value="<?php echo $client_id; ?>" selected><?php $cas=get_value_from_multi_object_array($client_id,$client_name,'client_id','client_name'); print_r($cas); ?></option>
                <?php }} ?>
           ...
           <form action="#" method="post" id="send_email" >
            ...
            if($client!=null){
                ?>
                    ...
                    <td class="left">
                                <label>
                                    <input type="checkbox" name="email[]" value="<?php echo $client->email; ?>">
                                    <span class="lbl"></span>
                                </label>
                        </td>
                        <td class="center">
                        ...
                        <td class="td-actions ">
                            <div class="hidden-phone visible-desktop btn-group">
                                <a title="Edit" class="btn btn-mini btn-info" href="<?php echo URL::to_route('EditClient').'/'.$client->client_id;  ?>">
                                    <i class="icon-edit bigger-120"></i>
                                </a>
                                <a title="Delete" class="btn btn-mini btn-danger" onclick="return confirm('Really want to delete this contact')" href="<?php echo URL::to_route('DeleteClient').'/'.$client->client_id;  ?>">
