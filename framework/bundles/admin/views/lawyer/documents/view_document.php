    ...
    <div class="clear-fix" id="page-content">
        <div class="page-header position-relative">
            <h1><i class="icon-book"></i>
                ...
                <a title="View All" href="<?php echo URL::to_route('ViewDocuments'); ?>" class="btn btn-mini btn-file" style="float: right">
                    ViewAll
                </a>
                </small>
            </h1>
...
<?php }?>
    ...
    <div class="row-fluid">
        ...
        <?php $search=libdocument::getdocumentByLawyerID(Auth::user()->id); ?>
            ...
            <form name="search" action="" method='get' onsubmit="return validation()">
                <select class='select2 span2' style="width:200px;" name="doc_name" onchange="this.form.submit()">
                    ...
                    <?php if($search){foreach($search as $docName){ if($docName->doc_name){ ?>
                ...
                <select class="select2 span2" style="width:200px;" name="case_no" onchange="this.form.submit()">
                    ...
                    <?php if($case=libdocument::caseNoByLawyerId(Auth::user()->id)){foreach($case as $caseNo){?>
                        <option value="<?php echo $caseNo->case_id; ?>" <?php echo (Input::old('case_no') == $caseNo->case_id)?'selected':''; ?>>
                        <?php echo $caseNo->case_no;  ?>
                        </option>
                        <?php }} ?>
                </select>
...
<form action="<?php echo URL::to_route('MultiDocumentDelete') ?>" method="post">
            ...
            <table id="table_bug_report" class="table table-striped table-bordered table-hover">
                ...
                <?php if (isset($document)) { $case=libcase::getcasedetail();
                    foreach ($document->results as $documents) { if($documents->lawyer_id==Auth::user()->id){ ?>
...
<?php echo $document->links();  ?>
    ...
    // function validation()
    // {
    //     var doc_name=document.search.doc_name.value;
    //     var case_no=document.search.case_no.value;
    //     var from_date=document.search.from_date.value;
    //     var to_date=document.search.to_date.value;
    // }
 $('span').tooltip();
  $('a').tooltip();
    </script>
