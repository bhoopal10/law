...
<div class="row-fluid">
    <form name="form2" action="<?php echo URL::to_route('SearchCase'); ?>" method="get">
        ...
        <select id="advocate" class="span2" name="updated_by" onchange='this.form.submit()'>
            ...
            <?php }} ?>
            <?php if(isset($search_case)){ $uniq=array(); foreach( $search_case as $case1){if(in_array($case1->updated_by,$uniq)){continue;}$uniq[]=$case1->updated_by;?>
...
<?php if (isset($cases->results)) {
    ...
    <tr>
        ...
        <td><a title="<?php echo $case->client_name . ' - Cases'; ?>" href="<?php echo URL::to("admin/report/client?id=$case->client_id"); ?>"><?php echo ucfirst($case->client_name); ?></a></td>
        <td><?php  $name=get_value_from_multi_object_array($case->associate_lawyer,$user,'id','first_name'); echo ucfirst($name); ?></td>
